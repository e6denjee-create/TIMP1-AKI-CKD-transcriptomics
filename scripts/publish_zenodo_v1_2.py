"""Create and publish Zenodo version 1.2.0 from the v1.1.0 record."""
...
PARTS_DIR = ROOT / "public_repository" / "zenodo_upload_parts_v1.2.0"
STATE = ROOT / "public_repository" / "zenodo_v1.2.0_state.json"
RESULT = ROOT / "public_repository" / "zenodo_v1.2.0_published_record.json"
API = "https://zenodo.org/api"
PREVIOUS_RECORD_ID = 20686503
...
def upload(client: requests.Session, url: str, path: Path) -> None:
    ...
    for attempt in range(1, 9):
        try:
            ...
            print(
                f"Upload attempt {attempt}/8 interrupted: "
                f"{type(error).__name__}. Retrying in {wait} seconds.",
                flush=True,
            )
            time.sleep(wait)
...
def main() -> None:
    ...
    upload_files = sorted(
        PARTS_DIR.glob("TIMP1_AKI_CKD_transcriptomics_zenodo_v1.2.0.zip.part*")
    )
    upload_files += [
    ...
    if len(upload_files) < 3:
        sys.exit("v1.2.0 upload parts are missing. Run prepare_zenodo_v1_2.py first.")
    ...
    if not state.get("deposition_id"):
        ...
        draft = (
            check(client.get(draft_url, timeout=120), "Load new-version draft")
            if draft_url
            else response
        )
        state = {
        ...
        print(
            f"Created version draft {state['deposition_id']}; "
            f"reserved DOI {state['doi']}",
            flush=True,
        )
    ...
    if not state.get("files_cleared"):
        for item in draft.get("files", []):
            check(
                client.delete(item["links"]["self"], timeout=120),
                f"Delete inherited file {item['filename']}",
            )
    ...
    metadata = {
        "metadata": {
            "title": (
                ...
                "Tubular Injury-Repair, Extracellular Matrix Remodeling, and a "
                "Reproducible Disease-State Program in Human Kidney Disease"
            ),
            "upload_type": "software",
            "description": (
                "Version 1.2.0 standardizes program scoring across the same external "
                "sample scope, adds a complete GEO search inventory and leave-one-"
                "signature-out injury-PC analyses, removes incomplete single-donor "
                "single-cell inference, and updates the manuscript and supplements."
            ),
            "creators": [
                {
                    "name": "Ji, Yanzhao",
            ...
            "related_identifiers": [
                ...
                },
                {
                    "identifier": (
                        "https://github.com/e6denjee-create/"
                        "TIMP1-AKI-CKD-transcriptomics/releases/tag/v1.2.0"
                    ),
